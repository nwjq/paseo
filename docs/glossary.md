...
- **Workspace** — One concrete `cwd` on one daemon, with git state; belongs to exactly one project. UI: "Workspace". Code: `WorkspaceDescriptorPayload` (`packages/server/src/shared/messages.ts:2178`). Behavior: workspace `cwd` is exact and stable (including git subdirectories) when opening projects and when creating/using worktrees; project grouping can still use git metadata like remote/main root. Don't confuse with: Branch (one branch can back many workspaces via worktrees). Forbidden: "Folder", "Directory" as UI label.
