...
describe("WorkspaceGitServiceImpl", () => {
  ...
  test("a failing repo-level fetch moves to a surviving workspace when its directory is gone", async () => {
    ...
    await vi.waitFor(() => {
      expect(runGitFetch).toHaveBeenCalledWith(deletedCwd, expect.anything());
    });
    await flushPromises();
    ...
    // Without the re-point the target would retry the deleted path on every interval.
    expect(runGitFetch.mock.lastCall?.[0]).toBe(REPO_CWD);
  ...
  test("a healthy repo-level fetch never probes the filesystem", async () => {
