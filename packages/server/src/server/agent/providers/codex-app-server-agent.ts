...
export class CodexAppServerAgentSession implements AgentSession {
  ...
  private async loadSkills(): Promise<void> {
    ...
    try {
      ...
      appServerSkills = Array.from(skillsByName.values());
    } catch (error) {
      this.logger.trace(
        {
          agentId: this.agentId,
    ...
    if (!appServerFailed) {
      // The app server owns skill semantics — including which skills are disabled — and it
      // already scans the exact session cwd, so never second-guess an answer it gave us.
      this.cachedSkills = appServerSkills;
      return;
    }

    // Only when the app server could not answer at all: scan the session cwd ourselves so a
    // workspace rooted in a repository subdirectory still surfaces its skills.
    let fallbackSkills: CodexSkillEntry[] = [];
    try {
    ...
    this.cachedSkills = fallbackSkills;
  }
