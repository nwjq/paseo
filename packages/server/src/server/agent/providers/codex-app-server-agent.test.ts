...
async function runCustomCodexProviderTurn(
  ...
  const workspaceDir = path.join(tempDir, "workspace", "project");
...
async function listCommandsFromFakeCodex(
  ...
  // The app-server is spawned with the session cwd, so it has to be a real directory.
...
describe("Codex app-server provider", () => {
  ...
  test("lists skills from the exact cwd instead of repo root", async () => {
    ...
    try {
      const commands = await listCodexSkills(cwd, workspaceGitService);
  ...
  test("falls back to the exact subdirectory cwd when the app server cannot list skills", async () => {
