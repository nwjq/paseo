...
export class WorkspaceGitServiceImpl implements WorkspaceGitService {
  ...
  private async runRepoFetch(target: RepoGitTarget): Promise<void> {
    ...
    try {
      ...
      await this.repointRepoFetchCwdIfGone(target);
    } finally {
  ...
  /**
   * The fetch cwd is pinned to whichever workspace created this target. That directory can be
   * deleted (an archived worktree) while other workspaces keep the target alive, which would
   * otherwise fail the repository's only background fetch on every interval, forever. Probe only
   * after a failure so healthy repositories never pay for the filesystem check.
   */
  private async repointRepoFetchCwdIfGone(target: RepoGitTarget): Promise<void> {
    ...
    for (const workspaceKey of target.workspaceKeys) {
      if (workspaceKey !== target.cwd && (await this.deps.directoryExists(workspaceKey))) {
        this.logger.debug(
          { repoGitRoot: target.repoGitRoot, from: target.cwd, to: workspaceKey },
          "Moving background git fetch to a surviving workspace directory",
        );
        target.cwd = workspaceKey;
        return;
      }
    }
  }
